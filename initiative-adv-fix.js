...
// Remove o carregador antigo da iniciativa, que podia ficar preso no getSession() em navegadores móveis.
...
const initiative=String.raw`(() => {
  let campaign=null,role='player',chars=[],channel=null,loading=false;
  const list=()=>document.querySelector('#initiativeSheetList');
  const badge=()=>document.querySelector('#initiativeRole');
  const esc=v=>String(v??'').replace(/[&<>'\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','\"':'&quot;'}[c]));
  const kind=k=>k==='player'?'Jogador':k==='npc'?'NPC':k==='monster'?'Monstro':'Outro';
  function uidFromSession(session){
    if(session?.user?.id)return session.user.id;
    try{const p=JSON.parse(atob(String(session?.access_token||'').split('.')[1].replace(/-/g,'+').replace(/_/g,'/')));return p?.sub||null}catch{return null}
  }
  async function restGet(table,query,session){
    const r=await withTimeout(fetch(SUPABASE_URL+'/rest/v1/'+table+'?'+query,{headers:{apikey:KEY,Authorization:'Bearer '+session.access_token},cache:'no-store'}),6000,'tempo limite da iniciativa');
    let body=null;try{body=await r.json()}catch{}
    if(!r.ok)throw new Error(body?.message||('erro '+r.status));return Array.isArray(body)?body:[];
  }
  async function restPatchCampaign(data,session){
    const q=new URLSearchParams({id:'eq.'+campaign.id});
    const r=await withTimeout(fetch(SUPABASE_URL+'/rest/v1/campaigns?'+q,{method:'PATCH',headers:{apikey:KEY,Authorization:'Bearer '+session.access_token,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify(data),cache:'no-store'}),6000,'tempo limite ao salvar iniciativa');
    if(!r.ok){let b=null;try{b=await r.json()}catch{}throw new Error(b?.message||('erro '+r.status))}
  }
  ...
  function showError(message){
    const l=list(),b=badge();if(b)b.textContent='Falha ao carregar';if(!l)return;
    l.innerHTML='<div class="initiative-load-error"><b>Não foi possível carregar a iniciativa.</b><div class="muted" style="margin-top:5px">'+esc(message||'Erro desconhecido')+'</div><button id="initRetryV21">Tentar novamente</button></div>';
    l.querySelector('#initRetryV21').onclick=load;
  }
  function render(){
    const l=list(),b=badge();if(!l||!b)return;const master=role==='master',arr=sorted();
    b.textContent=master?'Mestre · edição liberada':'Jogador · somente leitura';
    l.innerHTML=(arr.length?arr.map((it,i)=>'<div class="initiative-sheet-row"><div class="initiative-sheet-pos">'+(i+1)+'</div><div><div class="initiative-sheet-name">'+esc(it.name||'Sem nome')+'</div><div class="initiative-sheet-kind">'+kind(it.kind)+'</div></div>'+(master?'<input data-v21-value="'+esc(it.id)+'" type="number" value="'+(Number(it.value)||0)+'">':'<div class="initiative-sheet-value">'+(Number(it.value)||0)+'</div>')+(master?'<button data-v21-del="'+esc(it.id)+'" class="danger">×</button>':'<span></span>')+'</div>').join(''):'<div class="note">A iniciativa ainda não foi preenchida.</div>')+(master?'<div class="initiative-sheet-actions"><button id="v21Add" class="primary">+ Adicionar</button><button id="v21Clear" class="danger">Limpar</button></div>':'');
    if(master){
      l.querySelector('#v21Add').onclick=openAdd;
      l.querySelector('#v21Clear').onclick=()=>{if(items().length&&confirm('Limpar toda a iniciativa?'))persist([])};
      l.querySelectorAll('[data-v21-value]').forEach(inp=>{inp.onfocus=()=>inp.select();inp.onchange=()=>persist(items().map(x=>x.id===inp.dataset.v21Value?{...x,value:Number(inp.value)||0}:x))});
      l.querySelectorAll('[data-v21-del]').forEach(btn=>btn.onclick=()=>persist(items().filter(x=>x.id!==btn.dataset.v21Del));
    }
  }
  async function persist(next){
    if(role!=='master'||!campaign)return;const old=campaign.initiative;campaign.initiative=next;render();
    try{
      let q=await withTimeout(db.from('campaigns').update({initiative:next}).eq('id',campaign.id),5000,'tempo limite ao salvar iniciativa');
      if(q?.error)throw q.error;
    }catch(e){
      try{const session=activeSession||await sessionSafe();if(!session)throw new Error('sessão expirada');await restPatchCampaign({initiative:next},session)}catch(err){campaign.initiative=old;render();toast(err.message||'Não foi possível salvar a iniciativa.')}
    }
  }
  function openAdd(){
    const opts=chars.map(c=>'<option value="'+esc(c.id)+'">'+esc(c.name)+' — '+kind(c.kind)+'</option>').join('');
    const wrap=document.createElement('div');wrap.className='paper';wrap.style.cssText='position:fixed;z-index:1000;left:50%;top:50%;transform:translate(-50%,-50%);width:min(92vw,460px);box-shadow:0 18px 60px #0006';
    wrap.innerHTML='<h2>Adicionar à iniciativa</h2><div class="field"><label>Participante</label><select id="v21Char"><option value="">Outro / nome manual</option>'+opts+'</select></div><div class="field"><label>Nome manual</label><input id="v21Name" placeholder="Opcional"></div><div class="field"><label>Resultado do teste</label><input id="v21Value" type="number" value="0"></div><div class="row" style="margin-top:10px"><button id="v21Save" class="primary">Adicionar</button><button id="v21Cancel">Cancelar</button></div>';
    document.body.appendChild(wrap);wrap.querySelector('#v21Cancel').onclick=()=>wrap.remove();wrap.querySelector('#v21Save').onclick=()=>{const cid=wrap.querySelector('#v21Char').value||null,c=chars.find(x=>x.id===cid),custom=wrap.querySelector('#v21Name').value.trim(),name=custom||c?.name||'';if(!name)return toast('Escolha um participante ou digite um nome.');const item={id:crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).slice(2),character_id:cid,name,kind:c?.kind||'custom',value:Number(wrap.querySelector('#v21Value').value)||0};wrap.remove();persist([...items(),item])};
  }
  async function load(){
    if(loading)return;loading=true;const l=list(),b=badge();if(b)b.textContent='Carregando...';if(l)l.innerHTML='<div class="note">Carregando iniciativa...</div>';
    try{
      let tries=0;while(!row?.campaign_id&&tries++<80)await new Promise(r=>setTimeout(r,100));
      if(!row?.campaign_id)throw new Error('campanha da ficha não identificada');
      const session=activeSession||await sessionSafe();if(!session?.access_token)throw new Error('sessão expirada');const uid=uidFromSession(session);if(!uid)throw new Error('usuário da sessão não identificado');
      let campRows,memberRows,charRows;
      try{
        const [cq,mq,chq]=await Promise.all([
        ...
        if(cq.error)throw cq.error;campRows=[cq.data];memberRows=mq.error?[]:(mq.data?[mq.data]:[]);charRows=chq.error?[]:(chq.data||[]);
      }catch(e){
        const cq=new URLSearchParams({id:'eq.'+row.campaign_id,select:'id,name,initiative,owner_id'}).toString();
        const mq=new URLSearchParams({campaign_id:'eq.'+row.campaign_id,user_id:'eq.'+uid,select:'role'}).toString();
        const chq=new URLSearchParams({campaign_id:'eq.'+row.campaign_id,select:'id,name,kind'}).toString();
        [campRows,memberRows,charRows]=await Promise.all([restGet('campaigns',cq,session),restGet('campaign_members',mq,session).catch(()=>[]),restGet('characters',chq,session).catch(()=>[])]);
      }
      if(!campRows?.[0])throw new Error('campanha não encontrada ou sem permissão');campaign=campRows[0];chars=charRows||[];role=campaign.owner_id===uid?'master':(memberRows?.[0]?.role||'player');render();
      try{if(channel)db.removeChannel(channel);channel=db.channel('sheet-init-v21-'+row.campaign_id).on('postgres_changes',{event:'UPDATE',schema:'public',table:'campaigns',filter:'id=eq.'+row.campaign_id},p=>{if(p.new){campaign={...campaign,...p.new};render()}}).subscribe()}catch(e){console.warn('[Quimera iniciativa realtime]',e)}
    }catch(e){console.error('[Quimera iniciativa]',e);showError(e.message||'erro ao carregar')}finally{loading=false}
  }
  window.QuimeraReloadInitiative=load;
  load();
})();`;
new vm.Script(initiative,{filename:'sheet-initiative-v21.js'});
fs.writeFileSync(path.join(out,'sheet-initiative-v21.js'),initiative);

const advantageDescriptions=String.raw`(() => {
  const D={
    CON:[
      [
  ...
  function decorate(){
    ...
    document.querySelectorAll('#advantages .advattr').forEach((attrEl,ai)=>{
      const attr=attrs[ai];attrEl.querySelectorAll('.advgroup').forEach((groupEl,gi)=>{
        groupEl.querySelectorAll('.advitem').forEach((itemEl,ii)=>{
          if(itemEl.querySelector('.adv-desc-btn'))return;const info=D?.[attr]?.[gi]?.[ii];if(!info)return;
          const label=itemEl.querySelector('label');if(!label)return;const btn=document.createElement('button');btn.type='button';btn.className='adv-desc-btn';btn.textContent='Ver descrição';label.appendChild(btn);
          btn.onclick=e=>{e.preventDefault();e.stopPropagation();let panel=itemEl.querySelector('.adv-desc-panel');if(panel){panel.remove();btn.textContent='Ver descrição';return}panel=document.createElement('div');panel.className='adv-desc-panel';panel.innerHTML='<b>'+info[0]+'</b> — '+info[1];itemEl.appendChild(panel);btn.textContent='Fechar descrição'};
        });
      });
    });
  }
  const old=renderAdvantages;renderAdvantages=function(){old();decorate()};
  try{decorate()}catch(e){console.warn('[Quimera descrições]',e)}
})();`;
new vm.Script(advantageDescriptions,{filename:'sheet-adv-desc-v21.js'});
fs.writeFileSync(path.join(out,'sheet-adv-desc-v21.js'),advantageDescriptions);
